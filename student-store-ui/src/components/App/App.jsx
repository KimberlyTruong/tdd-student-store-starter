...
export default function App() {
	...
	const [purchases, setPurchases] = React.useState([]); // save all past purchases
	...
	const [checkoutForm, setCheckoutForm] = React.useState({
		name: "",
		email: "",
	}); // object (with attributes name and email)
	...
	const handleFiltering = (newCategory = category) => {
		...
		if (category != newCategory) {
			...
			if (category !== "All Categories") {
				const oldCategoryElement = document.getElementById(
					category.toLowerCase() + "-tag"
				);
				oldCategoryElement.classList.remove("category-selected");
			} else {
				const oldCategoryElement =
					document.getElementById("all-categories-tag");
				oldCategoryElement.classList.remove("category-selected");
			}
		}
		...
		allProducts.forEach((product) => {
			if (
				(product.name.toLowerCase().includes(searchInput.value) ||
					searchInput.value === "") &&
				(product.category === newCategory || newCategory === "All Categories")
			) {
	...
	const handleOnSubmitCheckoutForm = () => {
		...
		axios
			.post(`http://localhost:${PORT}/store`, {
				user: {
		...
		React.useEffect(async () => {
			/* Get all purchases from the store. */
			setIsFetching(true);
			const res = await axios.get(`http://localhost:${PORT}/purchases`);
